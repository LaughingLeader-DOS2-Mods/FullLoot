...
local function SessionLoading()
	...
	for preset,data in pairs(PRESETS) do
		...
		if skillset_data ~= nil then
			for _,skill in pairs(skillset_data) do
		...
		if skillset_data_act2 ~= nil then
			for _,skill in pairs(skillset_data_act2) do
		...
		if equipment_data ~= nil then
			for _,stat in pairs(equipment_data) do
				data.Equipment[#data.Equipment+1] = stat
				Ext.Print("[FullLoot:Bootstrap.lua] Added equipment stat (".. stat ..") to preset (".. preset ..").")
			end
		end
		...
		if equipment_data ~= nil then
			for _,stat in pairs(equipment_data) do
				data.Equipment_Act2[#data.Equipment_Act2+1] = stat
				Ext.Print("[FullLoot:Bootstrap.lua] Added equipment stat (".. stat ..") to preset (".. preset ..") (Act2).")
			end
		end
	end
end
